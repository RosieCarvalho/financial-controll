...
export default function FuturePlans() {
  const [plans, setPlans] = useState<FuturePlan[]>(MOCK_PLANS);
  ...
  const [plannedMonth, setPlannedMonth] = useState(new Date().getMonth().toString());
  const [plannedYear, setPlannedYear] = useState(new Date().getFullYear().toString());

  const filteredPlans = plans.filter(p => 
    p.itemName.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const calculateComfort = (totalVal: number, inst: number, startMonth: number, startYear: number) => {
    const installmentVal = totalVal / inst;
    const currentMonth = new Date().getMonth();
    const currentYear = new Date().getFullYear();
    
    // Difference in months from now to planned month
    const monthsWait = (startYear - currentYear) * 12 + (startMonth - currentMonth);
    
    // Estimated balance by the planned month
    const estimatedBalanceAtStart = MOCK_MONTHLY_STATS.currentBalance + (MOCK_MONTHLY_STATS.avgSurplus * Math.max(0, monthsWait));
    const balanceAfterPurchase = estimatedBalanceAtStart - (inst === 1 ? totalVal : installmentVal);
    ...
    if (!isComfortable) {
      if (inst > 1) {
        ...
        const monthsBuffer = Math.ceil(totalVal / MOCK_MONTHLY_STATS.avgSurplus);
        ...
        const monthsBuffer = Math.ceil((totalVal + MOCK_MONTHLY_STATS.avgExpenses - MOCK_MONTHLY_STATS.currentBalance) / MOCK_MONTHLY_STATS.avgSurplus);
        const suggestedDate = new Date();
        suggestedDate.setMonth(currentMonth + monthsBuffer);
        suggestedMonth = suggestedDate.getMonth();
        suggestedYear = suggestedDate.getFullYear();
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Planos para o Futuro</h1>
          <p className="text-muted-foreground">Planeje suas próximas conquistas com base na sua saúde financeira.</p>
        </div>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="sm:max-w-[425px]">
            ...
            <DialogFooter>
              ...
              <Button className="bg-emerald-600 hover:bg-emerald-700 text-white" onClick={handleAddPlan}>Simular</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
        ...
        {filteredPlans.length === 0 && (
          <div className="p-12 text-center border-2 border-dashed rounded-3xl bg-card/30">
            ...
            <p className="text-muted-foreground max-w-sm mx-auto">
              Comece a simular seus sonhos e veja quando é o melhor momento para realizá-los.
            </p>
            <Button 
              variant="outline" 
        ...
        <CardContent className="p-8 flex flex-col md:flex-row items-center gap-6">
          ...
          <div className="space-y-2 flex-1 text-center md:text-left">
            ...
            <p className="text-emerald-100 text-sm leading-relaxed max-w-2xl">
              Nosso sistema analisa sua <strong>sobra mensal média</strong> e seu <strong>saldo atual</strong>. Uma compra é considerada confortável se a parcela (ou o valor total) não comprometer mais de 40% da sua sobra mensal e se você mantiver uma reserva de segurança após a aquisição.
            </p>
          </div>
          <Button variant="secondary" className="bg-white text-emerald-700 hover:bg-emerald-50 font-bold shrink-0">
            Saiba Mais
          </Button>
        </CardContent>
      </Card>
