...
export default function Savings() {
  const [boxes, setBoxes] = useState<CashBox[]>(MOCK_SAVINGS);
  ...
  const handleOpenTransaction = (box: CashBox, type: 'deposit' | 'withdrawal') => {
    setSelectedBox(box);
    setTxType(type);
    setAmount("");
    setDescription("");
  ...
  const handleTransactionSubmit = () => {
    ...
    const newHistoryItem = {
      ...
      type: txType,
      description: description || (txType === 'deposit' ? 'Depósito' : 'Resgate')
    };
    ...
    toast.success(`${txType === 'deposit' ? 'Depósito' : 'Resgate'} realizado com sucesso!`);
  };
      ...
      <Dialog open={isTxDialogOpen} onOpenChange={setIsTxDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedBox?.name} • Saldo: {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(selectedBox?.balance || 0)}
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
        ...
        <DialogContent className="sm:max-w-lg max-h-[80vh] flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto pr-2 mt-4 space-y-3">
            {selectedBox?.history.map((item) => (
              <div key={item.id} className="flex items-center justify-between p-4 rounded-xl bg-secondary/30 hover:bg-secondary transition-colors">
                <div className="flex items-center gap-4">
                  <div className={cn(
                    "p-2 rounded-full",
                    item.type === 'deposit' ? "bg-emerald-100 text-emerald-600" : "bg-rose-100 text-rose-600"
                  )}>
                    {item.type === 'deposit' ? <ArrowUpRight className="h-4 w-4" /> : <ArrowDownRight className="h-4 w-4" />}
                  </div>
                  <div>
                ...
                <div className="text-right">
                  ...
                  <Badge variant="outline" className="text-[10px] h-4 py-0 flex items-center gap-1 mt-1">
          ...
          <DialogFooter className="pt-4 border-t mt-4">
            <Button variant="outline" className="w-full" onClick={() => setIsHistoryDialogOpen(false)}>Fechar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
